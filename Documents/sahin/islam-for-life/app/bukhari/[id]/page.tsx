...
import { bukhariBooks } from '@/lib/bukhariBooks';
...
export default function HadithReader() {
  ...
  // Determine Book Name for Header
  const bookInfo = bukhariBooks.find(b => b.id === Number(id));
  ...
  useEffect(() => {
    ...
    // If language is Bengali, fetch 'ben-bukhari'. Else 'eng-bukhari'.
    const edition = language === 'bn' ? 'ben-bukhari' : 'eng-bukhari';
  ...
  }, [id, language]); // Re-fetch if language changes
      ...
      <header className="sticky top-0 z-50 bg-zinc-950/90 backdrop-blur-xl border-b border-white/5 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{t('bukhari')}</span>
